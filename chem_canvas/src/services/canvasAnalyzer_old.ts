...
// Extract drawn/handwritten text from canvas image
...
[
  {
    "text": "The actual handwritten text you can read",
    "x": 100,
    "y": 200,
    "confidence": 0.95
  }
...
export const initializeWithProvidedAPIKey = (): void => {
  const providedApiKey = import.meta.env.VITE_GEMINI_API_KEY || '';
  storeAPIKey(providedApiKey);
};

// Analyze specific text content for inline corrections
export const analyzeTextContent = async (
  ...
  try {
    const analysisPrompt = `You are an expert ${subject} teacher. Analyze this text for corrections and provide character-level highlighting information.

TEXT TO ANALYZE: "${text}"

TASK: Identify errors, correct parts, and suggestions. For each issue, specify the exact character positions.

RESPONSE FORMAT:
Return ONLY a valid JSON array of corrections with this structure:
...
[
  {
    ...
    "x": 0,
    "y": 0,
    "message": "Specific correction message",
    "type": "error|warning|suggestion",
    "severity": "low|medium|high",
    "category": "formula|equation|notation|structure|general",
    "textShapeId": "${textShapeId}",
    "startChar": 0,
    "endChar": 5,
...
- Use "#ef4444" (red) for errors that need correction
- Use "#22c55e" (green) for parts that are correct
- Use "#f59e0b" (orange) for warnings/suggestions
- startChar and endChar should be the exact character indices in the text string
- For multi-word corrections, include the full span
- If the entire text is correct, return an empty array or a single positive correction

EXAMPLES:
- Error: "H2O is water" -> startChar: 0, endChar: 3, highlightColor: "#ef4444", message: "Incorrect formula - should be H₂O"
- Correct: "The reaction is exothermic" -> startChar: 0, endChar: 25, highlightColor: "#22c55e", message: "Correct statement"

Analyze the text and provide specific, actionable corrections with precise character positions.`;
    ...
    // Validate and clean the corrections
    return corrections.map((correction: any, index: number) => ({
      id: correction.id || `text-correction-${textShapeId}-${index}`,
      x: correction.x || 0,
      y: correction.y || 0,
      message: correction.message || '',
      type: correction.type || 'suggestion',
      ...
      highlightColor: correction.highlightColor || '#f59e0b'
    }));
