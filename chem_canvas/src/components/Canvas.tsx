...
export default function Canvas({
  ...
  const startDrawing = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    let activeTool = showChemistryToolbar ? chemistryTool : currentTool;

    // Override: If clicking on a shape, force 'move' tool behavior (unless using specific tools)
    // This ensures we can drag/select molecules even if the Pen tool is active
    if (clickedOnShape && !['eraser', 'rotate', 'textbox'].includes(activeTool)) {
      activeTool = 'move';
    }
  ...
  const draw = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (!isDrawing && !isDraggingShape && !isRotatingShape && !isResizing && !isRotating3DShape) {
      let foundHoveredId = null;
  ...
  return (
    ...
    >
      ...
      <canvas
        ref={canvasRef}
        className={`w-full h-full touch-none ${isDraggingShape || isRotatingShape || isRotating3DShape
          ? 'cursor-grabbing'
          : isResizing
            ? 'cursor-nwse-resize'
            : (showChemistryToolbar ? chemistryTool : currentTool) === 'move'
              ? (hoveredShapeId ? 'cursor-grab' : 'cursor-default')
              : (showChemistryToolbar ? chemistryTool : currentTool) === 'rotate'
                ? 'cursor-move'
                : 'cursor-crosshair'
          }`}
        style={{
