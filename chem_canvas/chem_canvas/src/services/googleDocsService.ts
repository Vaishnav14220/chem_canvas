...
export const exportResearchPaperToDocs = exportResearchPaperToGoogleDocs;
export const getRecentDocuments = listGoogleDocs;

// Export Deep Agent output to Google Docs
export async function exportDeepAgentOutputToDocs(
  ...
  if (conversationHistory && conversationHistory.length > 0) {
    fullContent = conversationHistory
      .map(msg => `**${msg.role === 'user' ? 'User' : 'Assistant'}**: ${msg.content}`)
      .join('\n\n');
  }
  
  return createGoogleDocWithContent(title, fullContent, 'markdown');
}

// Document info type alias
...
export default {
  ...
  // Aliases
  exportResearchPaperToDocs: exportResearchPaperToGoogleDocs,
  getRecentDocuments: listGoogleDocs,
