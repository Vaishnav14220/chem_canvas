...
import { Spinner } from './Studium/Spinner';
import { Sparkles, Info, Maximize2, RefreshCw, Edit3, CheckCircle, XCircle, HelpCircle, MousePointer2, X } from 'lucide-react';
...
interface LessonGeneratorActivityProps {
    onClose: () => void;
    apiKey?: string;
}

export const LessonGeneratorActivity: React.FC<LessonGeneratorActivityProps> = ({ onClose, apiKey }) => {
    ...
    const [quizCorrect, setQuizCorrect] = useState<string[]>([]);
        ...
        <div className="flex flex-col h-full space-y-6 overflow-auto">
            ...
            {(imageUrl || loading) && (
                <div className="flex flex-col lg:flex-row gap-6 animate-in fade-in duration-500">
                    {/* Image Canvas */}
                    <div className="flex-1 bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-center min-h-[400px] relative overflow-auto group select-none">
                        {imageUrl ? (
                            <div className="relative">
                                ...
                                {labels.map((label, idx) => {
                                    ...
                                    let borderClass = 'border-white/30 border-dashed';
