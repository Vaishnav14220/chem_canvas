...
This will be used to answer questions with accurate page citations.`
  ...
  const generateSummary = async (text: string): Promise<string> => {
    ...
    const prompt = `Please generate a comprehensive summary of the following document content.
    
    CRITICAL FORMATTING REQUIREMENTS:
    1. Output MUST be in valid Markdown format.
    2. Use clear headings (## for main sections, ### for subsections).
    3. Use bullet points for key takeaways and lists.
    4. For ALL mathematical formulas, equations, or variables, YOU MUST USE LaTeX syntax enclosed in dollar signs:
       - Inline math: $E = mc^2$
       - Block math: $$ \int_{0}^{\infty} x^2 dx $$
    5. Do NOT use [PAGE X] markers in the summary.
    6. Highlight important terms in **bold**.
    
    Structure the summary as follows:
    - **Executive Summary**: A brief overview.
    - **Key Concepts**: The main ideas presented.
    - **Detailed Analysis**: deeper dive into important sections.
    - **Conclusions**: Final thoughts or findings.

    Document Content:
    ${text.slice(0, 50000)}`; // Send first 50k chars to ensure we don't hit limits, though 1.5 Pro is large.
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const [text, metadataAnalysis, formulas, definitions] = await Promise.all([
      ...
      console.log('Document processing completed, text length:', text.length);
  ...
  return (
    <div className="fixed inset-0 z-50 flex bg-slate-950">
                  ...
                  >
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        disabled={isDisabled}
                        className={`flex-1 px-2 py-1.5 rounded-md text-[10px] font-medium transition-all relative ${hasContent
                      ...
                      <button
                        ...
                        className={`inline-flex items-center gap-0.5 rounded-md border border-yellow-400/30 px-2 py-1.5 text-[10px] font-medium transition-colors ${preparationMaterials[type]
      ...
      <div className="flex-1 flex flex-col">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between bg-black/20 backdrop-blur-xl border-b border-white/10 px-6 py-4 z-10">
          <div>
            <h2 className="flex items-center gap-3 text-lg font-semibold text-white tracking-tight">
            ...
            <p className="flex items-center gap-2 text-sm text-slate-400 mt-1">
              ...
              AI-powered analysis, extraction, and synthesis
            </p>
          </div>
          <div className="flex flex-wrap gap-3">
            <button
              onClick={onClose}
              className="inline-flex items-center gap-2 rounded-lg bg-white/5 border border-white/10 px-4 py-2 text-sm font-medium text-slate-300 hover:bg-white/10 hover:text-white transition-all duration-200 hover:shadow-lg hover:shadow-blue-500/10"
            >
        ...
        <div className="flex-1 overflow-hidden bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-[#0a0a0a] to-black text-slate-200 font-sans selection:bg-blue-500/30">
            ...
            <div className="flex-1 p-6 overflow-y-auto">
              <div className="max-w-4xl mx-auto space-y-6">
                ...
                <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8 shadow-xl">
                  ...
                  <div className="border-2 border-dashed border-white/10 rounded-xl p-12 text-center hover:border-blue-500/50 hover:bg-blue-500/5 transition-all duration-300 group cursor-pointer" onClick={() => !isProcessing && fileInputRef.current?.click()}>
                    <input
                      ref={fileInputRef}
                      type="file"
                      accept=".pdf"
                    ...
                    {isProcessing ? (
                      <div className="space-y-6">
                        ...
                        <div>
                          ...
                          <p className="text-blue-400 text-sm mt-2 animate-pulse font-medium">
                            {processingStatus}
                          </p>
                          <div className="mt-6 flex items-center justify-center gap-2 text-slate-400 text-xs bg-white/5 py-2 px-4 rounded-full inline-flex">
                      ...
                      <div className="space-y-6">
                        <div className="w-20 h-20 bg-blue-500/10 rounded-full flex items-center justify-center mx-auto group-hover:scale-110 transition-transform duration-300 border border-blue-500/20">
                        ...
                        <div>
                          ...
                          <p className="text-slate-400 text-sm mt-2 max-w-md mx-auto leading-relaxed">
                            Supports PDF files up to 50MB. We use advanced AI to extract text, understand context, and generate study materials.
                          </p>
                        </div>
                        <button
                          onClick={(e) => {
                          ...
                          className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-xl transition-all duration-200 shadow-lg shadow-blue-600/20 hover:shadow-blue-600/40 font-medium"
                        >
                ...
                {uploadedFiles.length > 0 && (
                  <div className="space-y-4">
                    ...
                    {uploadedFiles.map((doc) => (
                      <div key={doc.id} className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 hover:border-white/20 hover:bg-white/10 transition-all duration-300 group">
                        <div className="flex items-start justify-between mb-6">
                          <div className="flex gap-4">
                            ...
                            <div>
                              <h4 className="text-white font-semibold text-lg tracking-tight">{doc.name}</h4>
                              <p className="text-slate-400 text-sm mt-1 flex items-center gap-2">
                                <span className="w-1.5 h-1.5 rounded-full bg-green-500"></span>
                                Processed on {doc.uploadTime.toLocaleDateString()} at {doc.uploadTime.toLocaleTimeString()}
                              </p>
                            </div>
                          </div>
                          <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                            <button
                              ...
                              className="inline-flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 text-white px-4 py-2 rounded-lg text-sm font-medium transition-all"
                            >
                              <Download className="h-4 w-4" />
                              Summary (.md)
                            </button>
                        ...
                        <div className="bg-black/40 border border-white/5 rounded-xl p-5 relative overflow-hidden">
                          ...
                          <div className="flex items-center gap-2 mb-4">
                            <Sparkles className="h-4 w-4 text-blue-400" />
                            <span className="text-blue-200 font-medium text-sm uppercase tracking-wider">AI Summary</span>
                          </div>
                          <div className="text-slate-300 text-sm max-h-96 overflow-y-auto leading-relaxed pr-2 custom-scrollbar prose prose-invert prose-sm max-w-none">
                            <ReactMarkdown
                              ...
                              components={{
                                ...
                                code: ({ children }) => <code className="bg-slate-800 text-blue-300 px-1 py-0.5 rounded text-xs font-mono">{children}</code>,
                                blockquote: ({ children }) => <blockquote className="border-l-2 border-blue-500 pl-3 italic text-slate-400 my-2">{children}</blockquote>,
                              }}
                            >
                              {doc.summary}
                            </ReactMarkdown>
                  ...
                  {uploadedFiles.length > 0 ? (
                    <div className="space-y-4">
                      {uploadedFiles.slice(0, 3).map((doc, index) => (
                        <div key={doc.id} className="bg-white/5 border border-white/10 rounded-xl p-4 hover:bg-white/10 transition-colors duration-200">
                          <h4 className="text-white font-medium text-sm mb-2 truncate tracking-tight">{doc.name}</h4>

                          {doc.subject && (
                            <div className="mb-3">
                              <span className="text-[10px] text-slate-400 uppercase tracking-widest font-semibold">Subject</span>
                          ...
                          {doc.topics && doc.topics.length > 0 && (
                            <div className="mb-3">
                              <span className="text-[10px] text-slate-400 uppercase tracking-widest font-semibold mb-2 block">Topics</span>
                              <div className="flex flex-wrap gap-1.5">
                                {doc.topics.slice(0, 6).map((topic, topicIndex) => (
                                  <button
                                    key={topicIndex}
                                    onClick={() => handleTopicToggle(topic)}
                                    className={`inline-block text-[10px] px-2.5 py-1 rounded-full border transition-all duration-200 font-medium ${selectedTopics.includes(topic)
                                      ? 'bg-blue-600 text-white border-blue-500 shadow-lg shadow-blue-500/20 scale-105'
                                      : 'bg-blue-500/10 text-blue-300 border-blue-500/20 hover:bg-blue-500/20 hover:scale-105 hover:border-blue-500/30'
                                ...
                                {doc.topics.length > 6 && (
                                  <span className="text-[10px] text-slate-500 font-medium">+{doc.topics.length - 6} more</span>
                                )}
                              </div>
                            </div>
                          )}
                            ...
                            <div className="space-y-2 pt-2 border-t border-white/5">
                              {doc.metadata.fileSize && (
                                <div className="flex justify-between text-[10px]">
                                  ...
                                  <span className="text-slate-300 font-mono">{doc.metadata.fileSize}</span>
                                </div>
                              )}
                              {doc.metadata.estimatedPages && (
                                <div className="flex justify-between text-[10px]">
                                  <span className="text-slate-500">Pages</span>
                                  <span className="text-slate-300 font-mono">{doc.metadata.estimatedPages}</span>
                                </div>
                              )}
                              {doc.metadata.difficulty && (
                                <div className="flex justify-between text-[10px]">
                                  <span className="text-slate-500">Level</span>
                                  <span className="text-slate-300">{doc.metadata.difficulty}</span>
                                </div>
                              )}
                              {doc.metadata.type && (
                                <div className="flex justify-between text-[10px]">
                                  <span className="text-slate-500">Type</span>
                                  <span className="text-slate-300">{doc.metadata.type}</span>
                                </div>
                              )}
                            </div>
                          )}
                            ...
                            <div className="flex gap-2 mt-4 flex-wrap">
                              {Object.keys(preparationMaterials).map((type) => (
                                <button
                                  ...
                                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${activePreparationType === type
        ...
        {showTopicWorkspace && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
            <div className="bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-2xl p-8 max-w-7xl w-full mx-4 shadow-2xl max-h-[95vh] overflow-y-auto ring-1 ring-white/10">
              <div className="flex items-center justify-between mb-8">
                <div>
                  ...
                  <p className="text-slate-400 text-sm mt-2 ml-1">
                    Deep dive into concepts, generate materials, and visualize connections
                  </p>
                </div>
                <button
                  onClick={handleCloseTopicWorkspace}
                  className="text-slate-400 hover:text-white transition-all p-2 hover:bg-white/10 rounded-lg"
                >
                  <X className="h-6 w-6" />
                </button>
              </div>
              ...
              <div className="space-y-8">
                ...
                <div>
                  <h3 className="text-white font-medium mb-4 flex items-center gap-2 text-sm uppercase tracking-wider text-slate-400">
                    Selected Topics
                  </h3>
                  <div className="flex flex-wrap gap-3">
                    {selectedTopics.map((topic, index) => (
                      <div
                        key={index}
                        className="bg-blue-500/10 border border-blue-500/20 text-blue-200 px-5 py-2.5 rounded-xl shadow-lg shadow-blue-500/5 animate-in slide-in-from-bottom-4 duration-500 font-medium"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <button
                    ...
                    className="flex items-center gap-4 p-6 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 rounded-2xl transition-all duration-300 group disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-xl hover:shadow-blue-500/5 hover:-translate-y-1"
                  >
                    <div className="p-3 rounded-xl bg-blue-500/20 text-blue-400 group-hover:scale-110 transition-transform duration-300 border border-blue-500/30">
                    ...
                    <div className="text-left">
                      <div className="text-white font-semibold text-lg">Study Materials</div>
                      <div className="text-slate-400 text-sm mt-1">Generate comprehensive notes and resources</div>
                    </div>
                  </button>
                  ...
                  <button
                    ...
                    className="flex items-center gap-4 p-6 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 rounded-2xl transition-all duration-300 group hover:shadow-xl hover:shadow-green-500/5 hover:-translate-y-1"
                  >
                    <div className="p-3 rounded-xl bg-green-500/20 text-green-400 group-hover:scale-110 transition-transform duration-300 border border-green-500/30">
                      <Target className="h-6 w-6" />
                    </div>
                    <div className="text-left">
                      <div className="text-white font-semibold text-lg">Practice Quiz</div>
                      <div className="text-slate-400 text-sm mt-1">Test your knowledge with AI-generated questions</div>
                    </div>
                  </button>
                  ...
                  <button
                    ...
                    className="flex items-center gap-4 p-6 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 rounded-2xl transition-all duration-300 group hover:shadow-xl hover:shadow-purple-500/5 hover:-translate-y-1"
                  >
                    <div className="p-3 rounded-xl bg-purple-500/20 text-purple-400 group-hover:scale-110 transition-transform duration-300 border border-purple-500/30">
                      <Search className="h-6 w-6" />
                    </div>
                    <div className="text-left">
                      <div className="text-white font-semibold text-lg">Research Hub</div>
                      <div className="text-slate-400 text-sm mt-1">Deep dive exploration and academic sources</div>
                    </div>
                  </button>
                  ...
                  <button
                    ...
                    className="flex items-center gap-4 p-6 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 rounded-2xl transition-all duration-300 group disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-xl hover:shadow-orange-500/5 hover:-translate-y-1"
                  >
                    <div className="p-3 rounded-xl bg-orange-500/20 text-orange-400 group-hover:scale-110 transition-transform duration-300 border border-orange-500/30">
                      {isGeneratingMindMap ? (
                        <Loader2 className="h-6 w-6 animate-spin" />
                      ) : (
                    ...
                    <div className="text-left">
                      <div className="text-white font-semibold text-lg">Mind Map</div>
                      <div className="text-slate-400 text-sm mt-1">
                        {isGeneratingMindMap ? 'Generating visual map...' : 'Visualize connections and concepts'}
                      </div>
                    </div>
                  </button>
                </div>
        ...
        {showDocumentChat && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
            <div className={`bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-2xl h-[85vh] flex shadow-2xl transition-all duration-300 ring-1 ring-white/10 ${showPdfViewer ? 'w-full max-w-7xl' : 'w-full max-w-4xl'
              }`}>
              {/* Chat Section */}
              <div className={`flex flex-col transition-all duration-300 ${showPdfViewer ? 'w-2/5 border-r border-white/10' : 'w-full'
                }`}>
                {/* Header */}
                <div className="flex items-center justify-between p-6 border-b border-white/10 bg-white/5">
                  <div>
                    ...
                    <p className="text-slate-400 text-sm mt-1 ml-1">Ask questions and get answers with citations</p>
                  </div>
                  <button
                    onClick={() => {
                    ...
                    className="text-slate-400 hover:text-white transition-all p-2 rounded-lg hover:bg-white/10"
                  >
                    <X className="h-6 w-6" />
                  </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6 scroll-smooth">
                  {chatMessages.length === 0 ? (
                    <div className="text-center text-slate-400 mt-20">
                      ...
                      <p className="text-xl font-semibold text-white tracking-tight">Start a conversation</p>
                      <p className="text-sm mt-2 text-slate-500">Ask any question about your document to get started</p>
                      <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-3 max-w-2xl mx-auto">
                        <button
                          ...
                          className="bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 rounded-xl p-4 text-left text-sm text-slate-300 transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5"
                        >
                          <span className="block text-white font-medium mb-1">Main Topics</span>
                          <span className="text-xs text-slate-500">What are the main topics covered?</span>
                        </button>
                        <button
                          onClick={() => setChatInput("Can you summarize the key findings?")}
                          className="bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 rounded-xl p-4 text-left text-sm text-slate-300 transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5"
                        >
                          <span className="block text-white font-medium mb-1">Key Findings</span>
                          <span className="text-xs text-slate-500">Summarize the key findings</span>
                        </button>
                        <button
                          onClick={() => setChatInput("What are the important formulas or equations?")}
                          className="bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 rounded-xl p-4 text-left text-sm text-slate-300 transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5"
                        >
                          <span className="block text-white font-medium mb-1">Formulas</span>
                          <span className="text-xs text-slate-500">Show important formulas or equations</span>
                        </button>
                        <button
                          onClick={() => setChatInput("Explain the methodology used")}
                          className="bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 rounded-xl p-4 text-left text-sm text-slate-300 transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5"
                        >
                          <span className="block text-white font-medium mb-1">Methodology</span>
                          <span className="text-xs text-slate-500">Explain the methodology used</span>
                        </button>
                      </div>
                    </div>
                  ) : (
