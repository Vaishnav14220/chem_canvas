...
import { generateChatResponseStream } from '../services/chatService';
...
const ChatInterface: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const stream = await generateChatResponseStream(history, currentInput, currentImage || null, mode);
    ...
    <div className="flex flex-col h-full bg-slate-900/30 rounded-2xl border border-slate-800 overflow-hidden">
      ...
      <div className="p-3 border-b border-slate-800 bg-slate-900/50 flex items-center justify-between backdrop-blur">
        ...
        <div className="text-[10px] text-slate-500 font-mono hidden sm:block">
           {mode === 'FAST' && "Model: Gemini 2.5 Flash Lite"}
           {mode === 'PRO' && "Model: Gemini 3.0 Pro"}
           {mode === 'SEARCH' && "Model: Gemini 2.5 Flash + Google Search"}
        </div>
      </div>
          ...
          <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={`max-w-[85%] space-y-2 ${msg.role === 'user' ? 'items-end flex flex-col' : ''}`}>
              <div className={`p-4 rounded-2xl text-sm leading-relaxed ${
                ...
                {/* Grounding Sources Bubbles */}
                {msg.groundingMetadata?.groundingChunks && (
                  <div className="mt-4 pt-3 border-t border-slate-800/50">
                    <div className="flex items-center gap-2 text-slate-500 mb-2 text-[10px] font-mono uppercase tracking-wider">
                      ...
                      {msg.groundingMetadata.groundingChunks.map((chunk: any, idx: number) => {
                         ...
                         return (
                           <a 
                             ...
                             className="group bg-slate-900 hover:bg-slate-800 text-blue-400 border border-slate-700/50 hover:border-slate-600 px-3 py-1.5 rounded-lg text-xs transition-all flex items-center gap-1.5"
                           >
